...
public class TSP {
     ...
     */
    private static void dfs(int city, int n, Set<Integer> visited, int cost, int[][] graph, Result result ){
        if (visited.size() == n){
        ...
        for (int nextCity = 0; nextCity < graph[city].length; nextCity++) {
            ...
            dfs(nextCity, n, visited,cost + graph[city][nextCity], graph, result);
